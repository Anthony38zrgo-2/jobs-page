<template>
  ...
  <div class="flex flex-col bg-white min-h-screen max-w-md mx-auto shadow-2xl overflow-hidden">
    ...
    <div class="shrink-0 bg-amber-50 text-amber-900 border-b border-amber-200 p-3 text-xs flex items-center gap-2">
      <span class="material-icons text-amber-600 text-base shrink-0">warning</span>
      <span>
        Por tu seguridad, <strong>no compartas</strong> teléfonos, tarjetas,
        ni hagas pagos fuera de la plataforma.
      </span>
    </div>

    <!-- ── Chat header ───────────────────────────────────────────────────── -->
    <div class="shrink-0 flex items-center gap-3 px-4 py-3 border-b border-slate-100 bg-white">
      <button @click="router.back()" class="w-8 h-8 rounded-full hover:bg-slate-100 transition-colors flex items-center justify-center">
      ...
      <div class="flex-1 min-w-0">
        <p class="font-bold text-slate-800 text-sm truncate">{{ tech?.name ?? 'Técnico' }}</p>
        <p class="text-xs text-emerald-500 font-medium">En línea · S/. {{ tech?.price.toFixed(2) }}</p>
      </div>
    ...
    >
      ...
      <div class="flex justify-center">
        <span class="text-[11px] text-slate-400 bg-white px-3 py-1 rounded-full shadow-sm border border-slate-100">
        ...
        >
          ...
          <div
            ...
            :class="[
              ...
              msg.blocked ? 'bg-rose-100 text-rose-700 border border-rose-200' : '',
            ]"
          >
    ...
    <!-- ── Input bar ─────────────────────────────────────────────────────── -->
    <div class="shrink-0 bg-white border-t border-slate-100 px-4 py-3 flex items-center gap-2">
      <input
        v-model="draft"
...
// ─── Anti-bypass filter ──────────────────────────────────────────────────────
const BLOCKED_WORDS = [
  'celular', 'mi número', 'número', 'yape', 'plin', 'efectivo',
  'bcp', 'llámame', 'llama', 'whatsapp', 'tarjeta', 'interbank', 'bbva',
]
const PHONE_REGEX = /\b\d[\d\s\-]{6,}\d\b/g

function filterMessage(content: string): { filtered: string; blocked: boolean } {
  let blocked = false
...
async function send() {
  ...
  if (blocked) {
    ...
    alertTimer = setTimeout(() => { showBlockAlert.value = false }, 3500)
  }
  ...
  if (sentCount >= 2 && messages.value.filter(m => m.from === 'tech').length === 0) {
    ...
    setTimeout(async () => {
      ...
      addMessage({
        from: 'tech',
        content: 'Listo, ya voy en camino. Por favor ingresa el código cuando llegue.',
        blocked: false,
        timestamp: new Date(),
      })
      await scrollBottom()
...
onUnmounted(() => { clearTimeout(alertTimer) })
</script>

<style>
/* Message transition */
.msg-enter-active { transition: all 0.35s cubic-bezier(0.34, 1.56, 0.64, 1); }
.msg-enter-from   { opacity: 0; transform: translateY(16px) scale(0.95); }
